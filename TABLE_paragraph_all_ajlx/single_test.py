...
if __name__ == "__main__":
    ...
    # 开始时间
    start = time.time()
    days = common_.day_range('2019/07/01', '2019/07/01',1)
    ...
    for i in range(len(days)):
        ...
        # 单测
        sql1 = 'SELECT a.id, a.qw, a.ssjl, b.sb, a.is_table, b.wslx, b.spcx_id, a.pjjg,b.ajlx,a.ss ' \
                     'FROM ' \
                     '%s a left join ' \
                     '%s b on a.id=b.wenshu_id ' \
                     'where a.id=533530724146350727' %(config.Input_table["tb_wenshu"],config.Input_table["tb_wenshu_check"])
        print(sql1)

        # 更新
        sql_update = 'SELECT a.id, a.qw, a.ssjl, b.sb, a.is_table, b.wslx, b.spcx_id, a.pjjg,b.ajlx,a.ss ' \
               'FROM ' \
               '{} a left join ' \
               '{} b on a.id=b.wenshu_id ' \
               'where a.id in {}'.format(config.Input_table["tb_wenshu"], config.Input_table["tb_wenshu_check"], lines)

        # 读出的数据
        all_data = common_.db_connection(common_.db_connection_test, sql1)
