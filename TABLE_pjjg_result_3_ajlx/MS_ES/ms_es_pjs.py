...
class MS_ES_PJS():
    def get_PJJG(self, all_data):
        ...
        for data in all_data:    # a.id, a.pjjg, b.ajlx, b.wslx, b.spcx_id, a.sb, a.ssqq, c.pjjg as orin_pjjg
            if data[2] == 3 and data[3] == 1 and data[4] == 30200000000000000:
                if data[7]:
    ...
    def PJJG_classfication_(self, txt):
        ...
        # 驳回上诉，维持原判开头的
        if re.search('^驳回.*?维持|^驳回', txt) != None:
            v_temp['判决结果类型'] = '一审判决认定事实清楚，适用法律正确的'
            v_temp['判决结果'] = '判决驳回上诉，维持原判决'

        # 包含撤销、变更、依法改判但不包含维持
        elif re.search('撤销|变更|改判|增判', txt) and not re.search('维持', txt):
            v_temp['判决结果类型'] = '一审判决认定事实错误或者适用法律错误的'
            v_temp['判决结果'] = '判决依法改判、撤销或者变更'

        # 既包含维持又包含撤销、变更、依法改判 驳回
        elif re.search('维持', txt):
            if re.search('变更|撤销|改判|增判|驳回.*?(其余|其他|其它).{0,5}(诉求|诉讼请求|请求)', txt):
                v_temp['判决结果类型'] = '一审判决认定事实错误或者适用法律错误的'
                v_temp['判决结果'] = '判决部分改判、撤销或者变更'
            # 只包含维持，不包含撤销|变更|依法改判|驳回
            else:
                v_temp['判决结果类型'] = '一审判决认定事实清楚，适用法律正确的'
                v_temp['判决结果'] = '判决驳回上诉，维持原判决'

        # 不包含  撤销、变更、改判、维持, 但是包含驳回其他      (属于维持了部分原判决)
        elif not re.search('撤销|变更|改判|维持', txt) and re.search('驳回.*?(其余|其他|其它).{0,5}(诉求|诉讼请求|请求)', txt):
            v_temp['判决结果类型'] = '一审判决认定事实错误或者适用法律错误的'
            v_temp['判决结果'] = '判决部分改判、撤销或者变更'
