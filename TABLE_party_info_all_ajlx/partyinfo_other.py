...
class Party_extract_other():
    ...
    def get_party_info(self, all_data):  #id,party_info,ajlx
        ...
        for data in all_data:  # id,party_info,ajlx
            if data[1] and data[2] not in (2,3,4):
    ...
    '''获取头部中的原告'''
    def get_yg_bg_dsr(slef,dict_clean_party_info):

        dict_yg_bg_dsr = {}
        for k,v in dict_clean_party_info.items():
            if v:
                v = v.replace('(', '（').replace(')', '）').replace(':', '：').replace(' ', '')
                # 处理原被告没有标志不出现，但出现以下情况
                if re.search('（[0-9]+.*?原告.*?[0-9]+.*?被告）',v):
                    v_arr_ = []
                    v_arr = v.split('\n')

                    for i in range(len(v_arr)):
                        if re.search('（[0-9]+.*?原告.*?[0-9]+.*?被告）',v_arr[i]):
                            v_arr_.append('原告：'+v_arr[i])
                        elif re.search('（[0-9]+.*?被告.*?[0-9]+.*?原告）',v_arr[i]):
                            v_arr_.append('被告：' + v_arr[i])
                        else:
                            v_arr_.append(v_arr[i])
                    v = '\n'.join(v_arr_)
                # 处理再审抗诉机关，监督机关的情况
                elif re.search('(监督机关|抗诉机关)',v):
                    # 有正方
                    if re.search('\n(再审申请人|申请再审人|申诉人)',v):
                        pass
                    # 无正方(所有人全部为反方)--再审
                    else:
                        v = re.sub('(监督机关|抗诉机关)','原告：',v)
                        v = re.sub('\n原审(原告|被告)：','\n被告：',v)

                # 再审的特殊情况，抗诉机关无，无正方，其余全为反方
                elif re.search('(一审|原审)(原告|被告)、(二审|再审)(上诉人|被上诉人)',v):
                    v = '原告：\n'+re.sub('^(一审|原审)(原告|被告)、(二审|再审)(上诉人|被上诉人)','被告：', v)
                    v = re.sub('\n(一审|原审)(原告|被告)、(二审|再审)(上诉人|被上诉人)', '\n被告：', v)
                ...
                v = re.sub('一审.*?、.*?人', '被告', v)
                v = re.sub('：：', '：', v)
                v = re.sub('\n(原告|被告)(诉讼|委托)代理人：','委托诉讼代理人：', v)
                v = re.sub('代理人[0-9a-zA-Z一二三四五六七八九十`][：]{0,1}','代理人：', v)
                v = re.sub('原告[0-9a-zA-Z一二三四五六七八九十`][：]{0,1}', '原告：', v)
                v = re.sub('被告[0-9a-zA-Z一二三四五六七八九十`][：]{0,1}', '被告：', v)
                v = re.sub('^[0-9]+号', '', v)
                v = re.sub('\n[0-9]+号|\n两案', '\n', v)
                v = re.sub('（.*?(指派|委托)）', '', v)
                ...
                for i in range(len(rule_3.yg_pattern_1)):
                    if re.search(rule_3.yg_pattern_1[i], v) :
                        v_tmp['原告信息'] = re.search(rule_3.yg_pattern_1[i], v).group()
                        break
                    # 通知书的原告
                    else:
                        v_tmp['原告信息'] = v
                if not v_tmp['原告信息']:
                    for i in range(len(rule_3.yg_pattern_2)):
                        if re.search(rule_3.yg_pattern_2[i], v) :
                            v_tmp['原告信息'] = re.search(rule_3.yg_pattern_2[i], v).group()
                            break
                ...
                # 被告信息抽取
                if v_tmp['原告信息']:
                    v_tmp['被告信息'] = v.replace(v_tmp['原告信息'],'')
                    # 如果抽到被告
                    if v_tmp['被告信息']:
                        # 如果存在第三人
                        if re.search('\n.*?第三人[\S\s]+',v_tmp['被告信息']):
                            ...
                            bg_info = v_tmp['被告信息']
                            v_tmp['被告信息'] = re.sub(rule_3.dsr_pattern[0],'',bg_info)
                            # 顺带抽一下第三人信息
                            v_tmp['第三人信息'] = re.search(rule_3.dsr_pattern[0], bg_info).group()
                        # 如果不存在第三人
                        else:
                            v_tmp['被告信息'] = v_tmp['被告信息']
                    else:
                        v_tmp['被告信息'] = ''
                else:
                    v_tmp['被告信息'] = ''
                ...
                '''原代码'''
                # for j in range(len(rule_3.bg_pattern_2)):  #不带第三人
                #     if re.search(rule_3.bg_pattern_2[j], v) :
                #         v_tmp['被告信息'] = re.search(rule_3.bg_pattern_2[j], v).group()
                #         break
                # if not v_tmp['被告信息']:
                #     for j in range(len(rule_3.bg_pattern_1)):
                #         if re.search(rule_3.bg_pattern_1[j], v) :
                #             tail = 3
                #             v_tmp['被告信息'] = re.search(rule_3.bg_pattern_1[j], v).group()[:-tail]
                #             break
                #
                #
                # for q in range(len(rule_3.dsr_pattern)):
                #     try:
                #         v_tmp['第三人信息'] = re.search(rule_3.dsr_pattern[q], v).group()
                #     except:
                #         v_tmp['第三人信息'] = ''
                ...
                for i in range(len(v)):
                    if v[i][0] in role_title_arr:
                        ...
                        for title in role_title_arr:
                            p = '^{}'.format(title)
                            if re.search(p, v[i][0]) :
                                if not re.search('代理人|代表人',v[i][0]):
                                ...
                                elif  v[i][0]==title and len(v[i])>1:
                                    x_ = self.clean_name(v[i][1])
                                    name_arr.append(x_)
                                    address_arr.append('空')
                                    break
                                    ...
                                    x_ = self.clean_name(v[i][0])
                                    name_arr.append(x_)
                                    address_arr.append('空')
                                    break


                        # 当原被告有信息，但name_arr却为空，且所有title中都没出现，为了判断他是个人，看男女性别
                        if not name_arr and len(v[i])>2:
                            if v[i][1] in ['男', '女']:
                                x_ = self.clean_name(v[i][0])
                                name_arr.append(x_)
                                address_arr.append('空')
                        ...
                        # 找原被告的地址
                        if address_arr and len(v[i]) > 2:
                            for r in v[i]:
                                if re.search('住', r) and len(r) > 3:
                                    address_arr[-1] = r
                                    break
                                elif re.search('省|市|区|县|路|道', r) and not re.search('[事服]{1}务所|法律援助中心|法律服务', r):
                                        address_arr[-1] = r
                                        break
                    ...
                    if re.search('诉讼代理人|委托代理人',v[i][0]):
                        ...
                        if v[i][0][-3:]=='代理人' and len(v[i])>1:
                            ...
                            if re.search('、.*?[事服]{1}务所|法律援助中心|法律服务|该|系',tmp):
                                tmp = re.split('、', tmp)[0]
                            ...
                            if re.search('（|/\(', tmp):
                                if re.search('（.*?）|/\(.*?/\)',tmp):
                                    tmp = re.sub('（.*?）|/\(.*?/\)','',tmp)
                                elif re.search('（|/\(',tmp) and not re.search('）|/\)',tmp):
    ...
    def run(self,dict_party_info):
        ...
        yg_merged_dict_arr = self.correspond(dict_yg_arr, rule_3.yg_title)


        bg_merged_dict_arr = self.correspond(dict_bg_arr, rule_3.bg_title)

        dsr_merged_dict_arr = self.correspond(dict_dsr_arr, rule_3.dsr_title)
...
#     a = Party_extract3()
#     dict_bg_arr = [[{539580406714863009: '\n。\n郑宝珍，女，农民，住址辽宁省昌图县。\n'}]]
#     bg_merged_dict_arr = a.correspond(dict_bg_arr, rule_3.bg_title)
